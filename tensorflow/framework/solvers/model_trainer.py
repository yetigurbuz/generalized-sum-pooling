...
from ..metrics import factory as metrics
from .. import models
from .. import layers
...
class DMLFramework(object):
    ...
    def getModelName(self):
        ...
        loss = self.cfg.loss.function
        loss_texts = []
        is_wrapper = True
        while is_wrapper:
            if hasattr(self.cfg.loss.get(loss), 'function'):
                loss_texts.append(loss.upper() + '_')
                loss = self.cfg.loss.get(loss).function
            else:
                loss_texts.append(''.join([s.capitalize() for s in loss.split('_')]))
                is_wrapper = False
        loss_text = ''.join(loss_texts + ['Loss_'])

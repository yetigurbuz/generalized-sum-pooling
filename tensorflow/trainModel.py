...
def main():
    ...
    if dataset is not None:
        if 'cub' in dataset.lower():
            cfg.dataset.name = dataset = 'CUB200_2011'
        elif 'cars' in dataset.lower():
            cfg.dataset.name = dataset = 'Cars196'
        elif 'shop' in dataset.lower():
            cfg.dataset.name = dataset = 'InShop'
        elif 'sop' in dataset.lower():
            cfg.dataset.name = dataset = 'SOP'
        else:
            cfg.dataset.name = dataset


    # if dimension argument exists, update the fields
    if dim is not None:
        cfg.model.embedding_head.embedding_size = dim
    ...
    if dml_loss in ['proxy_nca', 'proxy_anchor', 'soft_triple']:
        ...
        cfg.model.embedding_head.GeneralizedSumPooling.warm_up_steps = cfg.training.steps_per_epoch
        cfg.model.embedding_head.GeneralizedSumPooling.entropy_regularizer_weight = 0.5

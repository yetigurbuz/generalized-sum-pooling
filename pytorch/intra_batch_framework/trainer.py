...
import logging
from intra_batch import net, data_utility
from intra_batch.RAdam import RAdam
from collections import defaultdict
import torch.nn as nn
from intra_batch.utils import losses, utils as utils
from intra_batch.net.gsp.zsr_loss import ZSRLoss
import torch
import random
import time
import torch.nn.functional as F
from intra_batch.evaluation import Evaluator_DML
...
class Trainer():
    ...
    def train(self):
        ...
        for i in range(self.num_iter):
            ...
            if self.config['train_params']['freeze_backbone']:
                utils.freeze_model(self.encoder,
                                   exclude=['embedding_head'])
            ...
            '''self.config['train_params']['is_apex'] = False
    ...
    def forward_pass(self, x, Y, I, P, train_params):
        ...
        # get normalized features and prediction probabilities
        x_probs, x_emb, x_attr = self.encoder(x.to(self.device), output_option=train_params['output_train_enc'])
 
        # Compute CE Loss
        loss = 0
        if self.ce:
            loss0 = self.ce(x_probs/self.config['train_params']['temperatur'], Y)
        ...
        if self.gnn_loss or self.of:
            edge_attr, edge_index, x_emb = self.graph_generator.get_graph(x_emb, Y)
            if type(loss) != int:
                loss = loss.cuda(self.device)
            pred, feats = self.gnn(x_emb, edge_index, edge_attr, train_params['output_train_gnn'])
        ...
        if self.triplet:
            triploss, _ = self.triplet(x_emb, Y)
        ...
        if self.distill:
            ...
            distill = self.distill(x_probs/self.config['train_params']['loss_fn']['soft_temp'], target)
        ...
        if self.of_pre:
            ...
            of_pre = self.of_pre(x_emb, target)
        ...
        if self.distance:
            ...
            dist = self.distance(teacher, x_emb)

            loss += train_params['loss_fn']['scaling_distance'] * dist
            self.losses['Distance'].append(dist.item())

        # Compute Zero-Shot Regularization Loss
        if self.zsr:
            zsr_loss = self.zsr(x_attr, Y) * train_params['loss_fn']['scaling_zsr']
            loss += zsr_loss
            self.losses['ZSR'].append(zsr_loss.item())
    ...
    def get_loss_fn(self, params, num_classes, emb_dim):
        ...
        # Zero-shot Regularization Loss
        if 'zsr' in params['fns'].split('_'):
            self.zsr = ZSRLoss(num_classes=num_classes,
                               emb_dim=emb_dim).to(self.device)
            print('\nINFO::TRAINING::LOSS: Using {} ...\n'.format(self.zsr.__class__.__name__))
        else:
            self.zsr = None
